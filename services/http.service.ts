...
// TYPES
import { IService, EHttpMethod } from "@/types/index";

class HttpService {
  private http: AxiosInstance;
  private baseUrl = process.env.NEXT_PUBLIC_BASE_URL;
  private gigBaseURL = process.env.NEXT_PUBLIC_GIG_BASE_URL;

  constructor(isBaseUrl = false) {
    this.http = axios.create({
      baseURL: isBaseUrl ? this.baseUrl : this.gigBaseURL,
  ...
  private get getAuthorization() {
    ...
    const accessToken = process.env.NEXT_PUBLIC_AUTHORIZATION
    return accessToken ? { Authorization: accessToken, sys_id: 6 } : { sys_id: 6 };
  }
